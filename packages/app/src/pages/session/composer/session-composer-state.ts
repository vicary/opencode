...
export function createDock(input: {
  todos: () => Todo[]
  active: () => boolean
  blocked: () => boolean
  closeMs: () => number
  ...
  const todo = createMemo(() => {
    ...
    const done = count > 0 && list.every((item) => item.status === "completed" || item.status === "cancelled")
  ...
  const live = createMemo(() => input.active() || input.blocked())

  const [store, setStore] = createStore({
    dock: todo().count > 0 && live(),
    closing: false,
    opening: false,
  })
  ...
  createEffect(
    on(
      () => [todo().count, todo().done, input.active(), input.blocked()] as const,
      ([count, done, active, blocked]) => {
